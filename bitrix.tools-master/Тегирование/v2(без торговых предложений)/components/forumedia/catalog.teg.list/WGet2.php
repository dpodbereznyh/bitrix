...
$filter=['IBLOCK_ID'=>$CATALOG_IBLOCK_ID,'SECTION_ID'=>$CATALOG_SECTION_ID];
  if($brand&&$prop!='BRAND'){$filter['PROPERTY_BRAND']=$brand;}
        ...
        foreach($list_prop as $code=>$props)
        {
            ...
            {   
                ...
                if($tegTemp['PROPERTIES']['LINK']['VALUE'][0]!="/"){
                    ...
                    if(($pos=strpos($curDir,'/filter/'))===false){
                        $tegTemp['PROPERTIES']['LINK']['VALUE']=$GLOBALS['APPLICATION']->GetCurDir().str_replace('#'.$code.'#',$code_val,trim($tegTemp['PROPERTIES']['LINK']['VALUE'],'/')).'/';
                        $tegTemp['PROPERTIES']['LINK']['~VALUE']=$GLOBALS['APPLICATION']->GetCurDir().str_replace('#'.$code.'#',$code_val,trim($tegTemp['PROPERTIES']['LINK']['~VALUE'],'/')).'/';
                    }else{
                ...
                $tegTemp['PROPERTIES']['NAME']['VALUE']=str_replace('#'.$code.'_NAME#',$prop['NAME'],$tegTemp['PROPERTIES']['NAME']['VALUE']);
                $tegTemp['PROPERTIES']['NAME']['~VALUE']=str_replace('#'.$code.'_NAME#',$prop['NAME'],$tegTemp['PROPERTIES']['NAME']['~VALUE']);
                $tegTemp['PROPERTIES']['H1']['VALUE']=str_replace('#'.$code.'_NAME#',$prop['NAME'],$tegTemp['PROPERTIES']['H1']['VALUE']);
                $tegTemp['PROPERTIES']['H1']['~VALUE']=str_replace('#'.$code.'_NAME#',$prop['NAME'],$tegTemp['PROPERTIES']['H1']['~VALUE']);
                $tegTemp['PROPERTIES']['TITLE']['VALUE']=str_replace('#'.$code.'_NAME#',$prop['NAME'],$tegTemp['PROPERTIES']['TITLE']['VALUE']);
                $tegTemp['PROPERTIES']['TITLE']['~VALUE']=str_replace('#'.$code.'_NAME#',$prop['NAME'],$tegTemp['PROPERTIES']['TITLE']['~VALUE']);
                $tegTemp['PROPERTIES']['DESCRIPTION']['VALUE']=str_replace('#'.$code.'_NAME#',$prop['NAME'],$tegTemp['PROPERTIES']['DESCRIPTION']['VALUE']);
                $tegTemp['PROPERTIES']['DESCRIPTION']['~VALUE']=str_replace('#'.$code.'_NAME#',$prop['NAME'],$tegTemp['PROPERTIES']['DESCRIPTION']['~VALUE']);
                $tegTemp['PROPERTIES']['TEXT_BOTTOM']['VALUE']=str_replace('#'.$code.'_NAME#',$prop['NAME'],$tegTemp['PROPERTIES']['TEXT_BOTTOM']['VALUE']);
                $tegTemp['PROPERTIES']['TEXT_BOTTOM']['~VALUE']=str_replace('#'.$code.'_NAME#',$prop['NAME'],$tegTemp['PROPERTIES']['TEXT_BOTTOM']['~VALUE']);
