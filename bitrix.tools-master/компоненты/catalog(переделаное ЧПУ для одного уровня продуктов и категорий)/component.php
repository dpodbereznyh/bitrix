...
//кастомизированная часть компонента отвечает за определение пренадлежности - элемент или секция на одном уровне.
if(empty($arVariables['SECTION_CODE'])&&$componentPage!='compare'&&$componentPage!='smart_filter'){
$componentPage = $engine->guessComponentPath(
		$arParams["SEF_FOLDER_DETAIL"],
		$arUrlTemplates,
		$arVariables
	);
...
$rsSections = CIBlockSection::GetList(array(),array('IBLOCK_ID' => $arParams['IBLOCK_ID'], '=CODE' =>$arVariables['SECTION_CODE'] ),false,false,['ID','CODE']);
	if (!$rsSections->Fetch()){
		$res = CIBlockElement::GetList(array(),array('IBLOCK_ID' => $arParams['IBLOCK_ID'], '=CODE' =>$arVariables['SECTION_CODE'] ),false,false,['ID','IBLOCK_SECTION_ID']);
		$el=$res->Fetch();
		$rsSections = CIBlockSection::GetList(array(),array('IBLOCK_ID' => $arParams['IBLOCK_ID'], '=ID' =>$el['IBLOCK_SECTION_ID'] ),false,false,['ID','CODE']);
		$sect=$rsSections->Fetch();
		$arVariables['ELEMENT_CODE']=$arVariables['SECTION_CODE'];
		$arVariables['SECTION_CODE']=$sect['CODE'];
	...
	/*		?>
<script>
	console.log(<?=json_encode($arVariables, JSON_UNESCAPED_UNICODE)?>);
	console.log(<?=json_encode($arUrlTemplates, JSON_UNESCAPED_UNICODE)?>);
console.log(<?=json_encode($sect, JSON_UNESCAPED_UNICODE)?>);
console.log(<?=json_encode($componentPage, JSON_UNESCAPED_UNICODE)?>);
</script>
	<?*/
