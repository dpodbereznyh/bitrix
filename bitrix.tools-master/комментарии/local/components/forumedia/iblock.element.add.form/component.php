...
$RECAPTCHA_SERVER_KEY=$arParams['RECAPTCHA_SERVER_KEY'];
...
if ($bAllowAccess)
{
	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		if ($arParams["USE_CAPTCHA"] == "Y" && $arParams["ID"] <= 0)
		{
 			...
 			function getCaptcha($secretKey)
            {
              $response = file_get_contents(
                "https://www.google.com/recaptcha/api/siteverify?secret=" . $RECAPTCHA_SERVER_KEY . "&response=" . $secretKey
              );
		...
		$url = "https://www.google.com/recaptcha/api/siteverify";
            ...
            $query = array(
        "secret" => $RECAPTCHA_SERVER_KEY, // Ключ для сервера
    ...
    // Если нет success то
    if (!$data['success']) {
        $arResult["ERRORS"][] ="Вы не прошли автоматическую проверку google";
	} else {}
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			else
			{
				...
				{
					$arResult["ERRORS"][] = $oElement->LAST_ERROR;
				}elseif(!empty($arParams["CATALOG_PROPERTY_COUNT"])){
					//расчет рейтинга
					$res=CIBlockElement::GetList([],['ID'=>$arParams["ELEM_ID"]],false,false,['IBLOCK_ID','ID','PROPERTY_'.$arParams["CATALOG_PROPERTY_COUNT"],'PROPERTY_'.$arParams["CATALOG_PROPERTY_SUM"]]);
					$ob=$res->GetNext();

					if(!empty($ob)){
						$sum=($ob['PROPERTY_'.strtoupper($arParams["CATALOG_PROPERTY_SUM"]).'_VALUE']+$_REQUEST['PROPERTY'][$arParams['ID_PROPERTY_STARS']][0]);
						$cnt=($ob['PROPERTY_'.strtoupper($arParams["CATALOG_PROPERTY_COUNT"]).'_VALUE']+1);

					CIBlockElement::SetPropertyValuesEx($arParams["ELEM_ID"], 
						$ob["IBLOCK_ID"], 
						array($arParams["CATALOG_PROPERTY_COUNT"] => $cnt));
					CIBlockElement::SetPropertyValuesEx($arParams["ELEM_ID"], 
						$ob["IBLOCK_ID"], 
						array($arParams["CATALOG_PROPERTY_SUM"] => $sum));
					CIBlockElement::SetPropertyValuesEx($arParams["ELEM_ID"], 
						$ob["IBLOCK_ID"], 
						array($arParams["CATALOG_PROPERTY_RATING"] => number_format($sum/$cnt,1,'.','')));
				}
				}
